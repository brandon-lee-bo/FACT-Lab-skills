...
> 分享说明：本文收录技能完整英文规则及自检清单。可将整份 Markdown 交给 AI，并附上待编辑文本；无需另外提供 `eval.md`。该技能面向一般写作；学术论文优先使用 paper-clarity-pfllm，以保留必要术语、技术性对比及论文结构。

使用示例（编辑）：

```text
请按这份文档中的 no-ai-slop 规则编辑以下文本，保留作者观点和个人语气。
返回完整修改稿及简短修改说明，不编造事实或例子。
[粘贴文本，并说明受众和发布场景]
```
...
```text
请按这份文档中的 Detect 模式检查以下文本，不直接改写。
逐项引用原句、指出表达问题并给出简短修改建议，不判断是否由 AI 创作。
[粘贴文本]
```
...
4. For an edit, make the minimum effective changes, then check the edited draft against [the included eval checklist](#appendix-eval) yourself.
